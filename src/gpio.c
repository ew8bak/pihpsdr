...
void gpio_restore_state()
{
  ...
  for (int i = 0; i < MAX_ENCODERS; i++)
  {
    ...
    if (value)
      encoders[i].bottom_encoder_enabled = atoi(value);
    sprintf(name, "encoders[%d].bottom_encoder_pullup", i);
    value = getProperty(name);
    if (value)
      encoders[i].bottom_encoder_pullup = atoi(value);
    sprintf(name, "encoders[%d].bottom_encoder_address_a", i);
    value = getProperty(name);
    if (value)
      encoders[i].bottom_encoder_address_a = atoi(value);
    sprintf(name, "encoders[%d].bottom_encoder_address_b", i);
    value = getProperty(name);
    if (value)
      encoders[i].bottom_encoder_address_b = atoi(value);
    sprintf(name, "encoders[%d].bottom_encoder_address_b", i);
    value = getProperty(name);
    sprintf(name, "encoders[%d].top_encoder_enabled", i);
    value = getProperty(name);
    if (value)
      encoders[i].top_encoder_enabled = atoi(value);
    sprintf(name, "encoders[%d].top_encoder_pullup", i);
    value = getProperty(name);
    if (value)
      encoders[i].top_encoder_pullup = atoi(value);
    sprintf(name, "encoders[%d].top_encoder_address_a", i);
    value = getProperty(name);
    if (value)
      encoders[i].top_encoder_address_a = atoi(value);
    sprintf(name, "encoders[%d].top_encoder_address_b", i);
    value = getProperty(name);
    if (value)
      encoders[i].top_encoder_address_b = atoi(value);
    sprintf(name, "encoders[%d].switch_enabled", i);
    value = getProperty(name);
    if (value)
      encoders[i].switch_enabled = atoi(value);
    sprintf(name, "encoders[%d].switch_pullup", i);
    value = getProperty(name);
    if (value)
      encoders[i].switch_pullup = atoi(value);
    sprintf(name, "encoders[%d].switch_address", i);
    value = getProperty(name);
    if (value)
      encoders[i].switch_address = atoi(value);
  }
  ...
  for (int f = 0; f < MAX_FUNCTIONS; f++)
  {
    for (int i = 0; i < MAX_SWITCHES; i++)
    {
      ...
      value = getProperty(name);
      if (value)
        switches_controller1[f][i].switch_enabled = atoi(value);
      sprintf(name, "switches[%d,%d].switch_pullup", f, i);
      value = getProperty(name);
      if (value)
        switches_controller1[f][i].switch_pullup = atoi(value);
      sprintf(name, "switches[%d,%d].switch_address", f, i);
      value = getProperty(name);
      if (value)
        switches_controller1[f][i].switch_address = atoi(value);
    }
  }
  ...
  if (controller != CONTROLLER1)
  {
    for (int i = 0; i < MAX_SWITCHES; i++)
    {
      ...
      value = getProperty(name);
      if (value)
        switches[i].switch_enabled = atoi(value);
      sprintf(name, "switches[%d].switch_pullup", i);
      value = getProperty(name);
      if (value)
        switches[i].switch_pullup = atoi(value);
      sprintf(name, "switches[%d].switch_address", i);
      value = getProperty(name);
      if (value)
        switches[i].switch_address = atoi(value);
    }
  }
}
...
void gpio_restore_actions()
{
  ...
  value = getProperty("controller");
  if (value)
    previous_controller = atoi(value);
  gpio_set_defaults(controller);
  ...
  if (controller == previous_controller)
  {
    if (controller != NO_CONTROLLER)
    {
      for (int i = 0; i < MAX_ENCODERS; i++)
      {
        ...
        value = getProperty(name);
        if (value)
          encoders[i].bottom_encoder_function = atoi(value);
        sprintf(name, "encoders[%d].top_encoder_function", i);
        value = getProperty(name);
        if (value)
          encoders[i].top_encoder_function = atoi(value);
        sprintf(name, "encoders[%d].switch_function", i);
        value = getProperty(name);
        if (value)
          encoders[i].switch_function = atoi(value);
      }
    }
    ...
    for (int f = 0; f < MAX_FUNCTIONS; f++)
    {
      for (int i = 0; i < MAX_SWITCHES; i++)
      {
        ...
        value = getProperty(name);
        if (value)
          switches_controller1[f][i].switch_function = atoi(value);
      }
    }
    if (controller == CONTROLLER2_V1 || controller == CONTROLLER2_V2)
    {
      for (int i = 0; i < MAX_SWITCHES; i++)
      {
        ...
        value = getProperty(name);
        if (value)
          switches[i].switch_function = atoi(value);
      }
    }
  }
}
