...
#ifdef _WIN32
void new_discover(int ifNum, u_long ifAddr, u_long ifNet_mask);
#else
...
void new_discovery()
{
    ...
    for (i = 0; i < (int)pIPAddrTable->dwNumEntries; i++)
    {
        ...
        new_discover(pIPAddrTable->table[i].dwIndex, (u_long)pIPAddrTable->table[i].dwAddr, pIPAddrTable->table[i].dwMask);
    }
#else
    getifaddrs(&addrs);
    ifa = addrs;
