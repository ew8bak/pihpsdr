...
void new_protocol_init(int pixels)
{
  ...
  if (command_response_sem_ready == SEM_FAILED)
    perror("CommandResponseReadySemaphore");
  sem_unlink("COMMRESBUF");
  command_response_sem_buffer = sem_open("COMMRESBUF", O_CREAT | O_EXCL, 0700, 0);
  if (command_response_sem_buffer == SEM_FAILED)
    perror("CommandResponseBufferSemaphore");
#else
  ...
  if (high_priority_sem_ready == SEM_FAILED)
    perror("HighPriorityReadySemaphore");
  sem_unlink("HIGHBUF");
  high_priority_sem_buffer = sem_open("HIGHBUF", O_CREAT | O_EXCL, 0700, 0);
  if (high_priority_sem_buffer == SEM_FAILED)
    perror("HIGHPriorityBufferSemaphore");
#else
  ...
  if (mic_line_sem_ready == SEM_FAILED)
    perror("MicLineReadySemaphore");
  sem_unlink("MICBUF");
  mic_line_sem_buffer = sem_open("MICBUF", O_CREAT | O_EXCL, 0700, 0);
  if (mic_line_sem_buffer == SEM_FAILED)
    perror("MicLineBufferSemaphore");
#else
  ...
  int optval = 1;
  setsockopt(data_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&optval, sizeof(optval));
...
static void new_protocol_high_priority()
{
  ...
  else
  {
    ...
    for (i = 0; i < receivers; i++)
    {
      ...
      ddc = i;
      if (device == NEW_DEVICE_ANGELIA || device == NEW_DEVICE_ORION || device == NEW_DEVICE_ORION2)
        ddc = 2 + i;
      int v = receiver[i]->id;
...
static void new_protocol_receive_specific()
{
  ...
  if (diversity_enabled && !isTransmitting())
  {
    ...
    receive_specific_buffer[25] = (receiver[0]->sample_rate / 1000) & 0xFF;
    ;                                     // sample rate LSB
    receive_specific_buffer[26] = 24;     // bits per sample
    receive_specific_buffer[1363] = 0x02; // sync DDC1 to DDC0
...
static gpointer high_priority_thread(gpointer data)
{
  ...
  while (1)
  {
    ...
    sem_wait(&high_priority_sem_buffer);
#endif
    process_high_priority(&high_priority_buffer);
