...
static void open_udp_socket()
{
  ...
  int optval = 1;
  if (setsockopt(tmp, SOL_SOCKET, SO_REUSEADDR, (char *) &optval, sizeof(optval)) < 0)
  {
  ...
  if (setsockopt(tmp, SOL_SOCKET, SO_RCVBUF, (char *) &optval, sizeof(optval)) < 0)
  {
      ...
      for (;;)
      {
        ...
        else if (data_socket >= 0)
        {
          ...
          if (bytes_read < 0 && errno != EAGAIN)
            perror("old_protocol recvfrom UDP:");
...
static int how_many_receivers()
{
  ...
  if (diversity_enabled)
    ret = 2; // need both RX channels, even if there is only one RX
  ...
  case RIGHT_SAMPLE_LOW:
    ...
    if (!isTransmitting() && diversity_enabled)
    {
      ...
      if (nreceiver + 1 == num_hpsdr_receivers)
      {
        ...
        // if we have a second receiver, display "auxiliary" receiver as well
        if (receivers > 1)
          add_iq_samples(receiver[1], left_sample_double_aux, right_sample_double_aux);
      }
    }
...
#ifdef PURESIGNAL
    if (isTransmitting() && transmitter->puresignal)
      i = receiver[PS_RX_FEEDBACK]->alex_antenna;
#endif
...
static void metis_start_stop(int command)
{
  ...
  if (device != DEVICE_OZY)
  {
    ...
    if (command == 0 && tcp_socket >= 0)
    {
      ...
      tmp = tcp_socket;
      tcp_socket = -1;
      // give some time to swallow incoming TCP packets
#ifdef _WIN32
      _sleep(100);
#else
    usleep(100000);
